...
const CATEGORIES = [
  "Uncategorized", "Groceries", "Transport", "Food", "Entertainment", "Rent", "Hobby", 
...
const WidgetWrapper = ({ title, icon: Icon, onClick, children, isPremium = false, isEditing = false, onRemove = undefined, draggable = false, onDragStart, onDragEnter, onDragEnd }: any) => (
  <div 
    ...
    className={`bg-white p-5 rounded-2xl shadow-sm border transition-all h-full flex flex-col relative overflow-hidden group
      ${isEditing ? 'border-dashed border-2 border-blue-300 scale-[0.98] cursor-grab active:cursor-grabbing hover:bg-blue-50/50' : 'border-gray-100 hover:shadow-md hover:border-gray-300 cursor-pointer'}
    `}
  >
    {isEditing && (
      <div className="absolute top-0 left-0 w-full h-full z-10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-white/40 backdrop-blur-[1px]">
         <div className="bg-white text-blue-600 p-2 rounded-full shadow-lg flex items-center gap-2 font-semibold text-sm">
            <GripHorizontal size={18} /> Drag to move
         </div>
      </div>
    )}
    ...
    {isEditing && onRemove && (
      <button 
        ...
        className="absolute top-3 right-3 bg-red-100 text-red-600 p-1.5 rounded-full z-20 hover:bg-red-200 transition-colors shadow-sm"
      >
    ...
    <div className="flex justify-between items-center mb-4 relative z-0">
      {/* Softened font from bold to semibold */}
      <h3 className="font-semibold text-gray-700 flex items-center gap-2 text-sm uppercase tracking-wider">
        {Icon && <Icon size={16} className={isEditing ? "text-blue-400" : "text-gray-400"} />}
        {title}
      </h3>
      {!isEditing && (
        isPremium ? (
          <span className="bg-gradient-to-r from-amber-200 to-yellow-400 text-yellow-900 text-[10px] font-bold uppercase px-2 py-1 rounded-full flex items-center gap-1 shadow-sm">
            <Lock size={10} /> Pro
          </span>
        ) : (
          onClick && <ArrowUpRight size={18} className="text-gray-300 group-hover:text-black transition-colors" />
        )
      )}
    </div>
    <div className={`flex-1 flex flex-col justify-center relative z-0 ${isEditing ? 'opacity-50 grayscale transition-all' : ''}`}>
      {children}
    </div>
  </div>
);
...
export default function Dashboard() {
  ...
  const [visibleWidgets, setVisibleWidgets] = useState<string[]>(["netWorth", "portfolio", "switches", "monthly", "upcoming", "recent"]);
  const [draggedWidgetIdx, setDraggedWidgetIdx] = useState<number | null>(null);

  // AUTO-FETCH LIVE PRICES ON LOAD
  const [hasAutoFetched, setHasAutoFetched] = useState(false);
  useEffect(() => {
    // Only run this once the local data is loaded, there's actually a portfolio, and we haven't fetched yet
    if (isLoaded && portfolio.length > 0 && !hasAutoFetched) {
      refreshPrices();
      setHasAutoFetched(true);
    }
  }, [isLoaded, portfolio.length, hasAutoFetched]);
  ...
  useEffect(() => {
    ...
    if (n.includes("tesco") || n.includes("aldi") || n.includes("asda") || n.includes("sainsbury")) {
      setCategory("Groceries");
    } else if (n.includes("uber") || n.includes("train") || n.includes("tfl") || n.includes("petrol")) {
      setCategory("Transport");
    } else if (n.includes("mcdonalds") || n.includes("kfc") || n.includes("restaurant") || n.includes("coffee") || n.includes("starbucks") || n.includes("deliveroo")) {
      setCategory("Food");
    } else if (n.includes("netflix") || n.includes("spotify") || n.includes("amazon prime")) {
      setCategory("Subscriptions");
    } else if (n.includes("rent") || n.includes("mortgage")) {
      setCategory("Rent");
    } else if (n === "") {
  ...
  if (!isLoaded) return <div className="h-screen w-full flex items-center justify-center bg-gray-50 text-gray-400">Loading your finances...</div>;
  ...
  const groupedPortfolio = filteredPortfolio.reduce((acc, item) => {
    if (!acc[item.symbol]) {
        acc[item.symbol] = { symbol: item.symbol, name: item.name, type: item.type, imageUrl: item.imageUrl, totalAmount: 0, totalCost: 0, lots: [] };
    }
  ...
  const topHoldings = [...groupedAssets]
    .map(asset => {
        ...
        return {
            ...asset,
            liveValue: asset.totalAmount * currentPriceToUse,
            isProfit: (asset.totalAmount * currentPriceToUse) - asset.totalCost >= 0
        }
    })
    .sort((a, b) => b.liveValue - a.liveValue)
    .slice(0, 3);

  const totalPortfolioValue = topHoldings.length > 0 ? groupedAssets.reduce((sum, asset) => {
      const avgPrice = asset.totalAmount > 0 ? (asset.totalCost / asset.totalAmount) : 0;
      const currentPriceToUse = livePrices[asset.symbol] || avgPrice;
      return sum + (asset.totalAmount * currentPriceToUse);
  }, 0) : 0;
  ...
  const activeSwitches = switches
    .filter(sw => sw.status === "active")
    .sort((a, b) => new Date(a.requirements.payInDeadline).getTime() - new Date(b.requirements.payInDeadline).getTime());
  ...
  const handleAddPortfolio = () => {
    ...
    addPortfolioItem({ 
      name: portName, symbol: portType === "crypto" ? portSymbol.toLowerCase().trim() : portSymbol.toUpperCase().trim(), 
      amount: parseFloat(portAmount), purchasePrice: parseFloat(portPurchasePrice), imageUrl: portImageUrl, type: portType, date: portDate, platform: portPlatform.trim()
    });
  ...
  const handleMonthChange = (direction: "prev" | "next") => {
    ...
    setIsEditingBudget(false); setEditingTxId(null);
  };
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `wealthbase_backup_${format(new Date(), "yyyy-MM-dd")}.json`;
    a.click();
  };
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-xl p-4 mt-2 shadow-lg absolute z-20 w-full max-w-[320px] top-full left-0">
            <div className="flex justify-between items-center mb-4">
                ...
                <span className="font-semibold text-gray-800">{format(pickerDate, "MMMM yyyy")}</span>
                    ...
                    return (
                        <button
                            ...
                            className={`h-9 w-9 text-sm rounded-full flex items-center justify-center transition
                                ${isSelected ? "bg-black text-white font-semibold shadow-md" : "text-gray-700 hover:bg-gray-100"}
                                ${isFutureDate ? "opacity-20 cursor-not-allowed" : "cursor-pointer"}
                                ${isDiffMonth ? "opacity-30 text-gray-400" : ""}
                                ${isTodayDate && !isSelected ? "ring-2 ring-black font-semibold" : ""}
                            `}
                        >
                            {format(day, "d")}
                        </button>
  ...
  const NavButton = ({ id, label }: { id: typeof activeTab, label: string }) => (
    <button 
      onClick={() => { setActiveTab(id); setError(""); setEditingTxId(null); }} 
  ...
  const renderWidget = (id: string, index: number) => {
    const dragProps = isEditingDashboard ? {
      draggable: true,
      ...
      case "netWorth":
        return (
          <WidgetWrapper key={id} title="Total Net Worth" icon={Wallet} onClick={() => setActiveTab("portfolio")} isEditing={isEditingDashboard} onRemove={() => toggleWidget("netWorth")} {...dragProps}>
            {/* Softened font-black to font-semibold */}
            <h2 className="text-4xl font-semibold text-gray-900 tracking-tight">£{netWorth.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</h2>
            <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-50">
              <div>
                <p className="text-xs text-gray-400 uppercase font-semibold tracking-wider mb-0.5">Cash</p>
                <p className="font-medium text-gray-700">£{totalCash.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase font-semibold tracking-wider mb-0.5">Invested</p>
                <p className="font-medium text-gray-700">£{totalPortfolioValue.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
              </div>
            </div>
          </WidgetWrapper>
        );
        ...
        return (
          <WidgetWrapper key={id} title={`This Month (${format(selectedMonth, "MMM")})`} icon={PieIcon} onClick={() => setActiveTab("daily")} isEditing={isEditingDashboard} onRemove={() => toggleWidget("monthly")} {...dragProps}>
            <div className="flex justify-between items-end">
              <div>
                <p className="text-sm text-gray-500 mb-1 font-medium">Spent so far</p>
                <h3 className="text-2xl font-semibold text-red-600">-£{stats.expense.toFixed(2)}</h3>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-500 mb-1 font-medium">Income</p>
                <h3 className="text-xl font-semibold text-green-600">+£{stats.income.toFixed(2)}</h3>
              </div>
            </div>
            <div className="w-full bg-gray-100 h-2 rounded-full mt-4 overflow-hidden flex">
              <div className="bg-red-500 h-full rounded-l-full" style={{ width: `${Math.min((stats.expense / (stats.income || 1)) * 100, 100)}%` }}></div>
            </div>
          </WidgetWrapper>
        );
        ...
        return (
          <WidgetWrapper key={id} title="Top Holdings" icon={BarChart3} onClick={() => setActiveTab("portfolio")} isEditing={isEditingDashboard} onRemove={() => toggleWidget("portfolio")} {...dragProps}>
            {topHoldings.length === 0 ? (
               <p className="text-gray-400 text-sm mt-4 text-center">No investments added.</p>
            ) : (
               <div className="space-y-3 mt-2">
                 {topHoldings.map(asset => (
                   <div key={asset.symbol} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                     <div className="flex items-center gap-3">
                        {asset.imageUrl ? (
                            <img src={asset.imageUrl} alt={asset.name} className="w-6 h-6 rounded-full border border-gray-100 shadow-sm" />
                        ) : (
                            <div className={`w-6 h-6 flex items-center justify-center rounded-full text-[8px] font-semibold uppercase tracking-wider shadow-sm ${asset.type === 'crypto' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                                {asset.symbol.substring(0, 2)}
                            </div>
                        )}
                        <p className="font-medium text-gray-700 leading-none truncate max-w-[120px]">{asset.name}</p>
                     </div>
                     <div className="text-right">
                        <span className="font-semibold text-gray-900 block">£{asset.liveValue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                     </div>
                   </div>
                 ))}
               </div>
        ...
        return (
          <WidgetWrapper key={id} title="Active Bonuses" icon={Clock} onClick={() => setActiveTab("switches")} isEditing={isEditingDashboard} onRemove={() => toggleWidget("switches")} {...dragProps}>
             {activeSwitches.length === 0 ? (
               <p className="text-gray-400 text-sm mt-4 text-center">No active bank switches.</p>
             ) : (
               <div className="space-y-3 mt-2">
                 {activeSwitches.slice(0, 3).map(sw => {
                   ...
                   return (
                     <div key={sw.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                       <div className="flex items-center gap-3">
                          <div className={`w-2 h-2 rounded-full shadow-sm ${isUrgent ? 'bg-red-500 animate-pulse' : 'bg-blue-500'}`}></div>
                          <div>
                            <p className="font-medium text-gray-700 leading-none">{sw.bankName}</p>
                            <p className={`text-[10px] uppercase font-semibold mt-1 ${isUrgent ? 'text-red-500' : 'text-gray-400'}`}>
                              {daysLeft < 0 ? 'Passed' : `${daysLeft} Days Left`}
                            </p>
                          </div>
                       </div>
                       <span className="font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">£{sw.bonusAmount}</span>
                     </div>
                   )
                 })}
               </div>
        ...
        return (
          <WidgetWrapper key={id} title="Upcoming Payments" icon={CalendarIcon} onClick={() => setActiveTab("subscriptions")} isEditing={isEditingDashboard} onRemove={() => toggleWidget("upcoming")} {...dragProps}>
            {upcomingSubs.length === 0 ? (
              <p className="text-gray-400 text-sm mt-4 text-center">No subscriptions added.</p>
            ) : (
              <div className="space-y-3 mt-2">
                {upcomingSubs.slice(0, 3).map(sub => {
                  let diff = sub.billingDay - todayDay; 
                  if (diff < 0) diff += 30;
                  return (
                    <div key={sub.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full shadow-sm ${diff <= 2 ? 'bg-red-500' : 'bg-orange-400'}`}></div>
                        <div>
                          <p className="font-medium text-gray-700 leading-none">{sub.name}</p>
                          <p className="text-[10px] text-gray-400 uppercase font-semibold mt-1">
                            {diff === 0 ? 'Due Today' : `Due in ${diff} days`}
                          </p>
                        </div>
                      </div>
                      <span className="font-semibold text-gray-900">£{sub.cost.toFixed(2)}</span>
                    </div>
                  )
                })}
              </div>
          ...
          <WidgetWrapper key={id} title="Recent Activity" icon={Activity} onClick={() => setActiveTab("daily")} isEditing={isEditingDashboard} onRemove={() => toggleWidget("recent")} {...dragProps}>
            <div className="space-y-3 mt-2">
              {transactions.length === 0 ? (
                <p className="text-gray-400 text-sm mt-4 text-center">No transactions logged.</p>
              ) : (
                transactions.slice(0, 3).map(tx => (
                  <div key={tx.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                    <div className="flex items-center gap-3">
                        <div className={`p-1.5 rounded-full ${tx.type === 'income' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                          {tx.type === 'income' ? <ArrowUpRight size={12}/> : <ArrowDownLeft size={12}/>}
                        </div>
                        <div>
                          <p className="font-medium text-gray-700 leading-none">{tx.note || tx.category}</p>
                          <p className="text-[10px] text-gray-400 font-semibold mt-1">{format(parseISO(tx.date), "MMM do")}</p>
                        </div>
                    </div>
      ...
      case "ai":
        return (
          <WidgetWrapper key={id} title="AI Wealth Insights" icon={Sparkles} isPremium={true} isEditing={isEditingDashboard} onRemove={() => toggleWidget("ai")} {...dragProps}>
            <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-5 rounded-xl border border-indigo-100 shadow-inner group-hover:shadow-md transition-shadow relative overflow-hidden h-full flex flex-col justify-center">
              ...
              <p className="text-sm text-indigo-950 font-medium leading-relaxed relative z-10">
                  ...
                  Moving this to a 5.1% High-Yield Cash ISA could earn you an extra <span className="font-semibold text-indigo-700 border-b border-indigo-200 pb-0.5 cursor-pointer">£{((totalCash > 0 ? totalCash : 5000) * 0.051).toFixed(0)}/yr</span>.
              </p>
              <button className="mt-4 text-xs font-semibold bg-indigo-600 hover:bg-indigo-700 transition-colors text-white px-4 py-2.5 rounded-lg w-full shadow-sm flex items-center justify-center gap-2 relative z-10">
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex flex-col md:flex-row justify-between items-center gap-3 md:gap-8">
          ...
          <div className="flex items-center justify-between w-full md:w-auto">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold tracking-tight text-gray-900">WealthBase</h1>
            </div>
            
            <div className="md:hidden flex items-center gap-3">
              <button onClick={exportData} className="text-gray-400 hover:text-black transition">
                 <DownloadCloud size={20} />
              </button>
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-white font-semibold text-xs shadow-sm cursor-pointer">
                JS
              </div>
            </div>
          </div>
          ...
          <nav className="flex w-full md:w-auto overflow-x-auto hide-scrollbar gap-1 pb-1 md:pb-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
             ...
             <NavButton id="daily" label="Daily Activity" />
          ...
          <div className="hidden md:flex items-center gap-4">
             <button onClick={exportData} className="text-gray-400 hover:text-black transition flex items-center gap-1.5 text-sm font-medium">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="mb-8">
          <h2 className="text-3xl font-semibold text-gray-900">{greeting} 👋</h2>
          ...
          <div className="animate-in fade-in duration-300">
            ...
            <div className="flex justify-between items-center mb-6">
               ...
               <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${isEditingDashboard ? 'bg-blue-600 text-white shadow-md' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
               >
            ...
            {isEditingDashboard && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-5 mb-6 animate-in fade-in slide-in-from-top-2 shadow-inner">
                <h3 className="font-semibold text-blue-900 text-sm uppercase tracking-wider mb-3">Add Widgets to Dashboard</h3>
                    ...
                    return (
                      <button 
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium border flex items-center gap-2 transition-all
                          ${isAdded 
                            ? 'bg-white/50 border-blue-200/50 text-blue-300 cursor-not-allowed' 
                            : 'bg-white border-blue-200 text-blue-700 hover:bg-blue-100 hover:shadow-sm'}`}
                      >
                        <Plus size={16} /> {w.label}
                      </button>
                    )
                  })}
                </div>
            ...
            {visibleWidgets.length === 0 && !isEditingDashboard && (
               <div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-2xl bg-white">
                  ...
                  <button onClick={() => setIsEditingDashboard(true)} className="bg-black text-white px-6 py-2 rounded-lg font-medium hover:bg-gray-800">Add Widgets</button>
               </div>
            )}
        ...
        {(activeTab === "daily" || activeTab === "analytics") && (
          <div className="mb-8">
              <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-4">
                  ...
                  <h2 className="text-xl font-semibold text-gray-900">{format(selectedMonth, "MMMM yyyy")}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  {isEditingBudget ? (
                      ...
                      <input type="number" value={tempBudget} onChange={(e) => setTempBudget(e.target.value)} className="w-full p-1 border rounded text-lg font-semibold" placeholder="Base Budget" autoFocus />
                      ...
                      <>
                      <h3 className={`text-2xl font-semibold ${budgetLeft < 0 ? 'text-red-600' : 'text-emerald-600'}`}>£{budgetLeft.toFixed(2)}</h3>
                  ...
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100"><p className="text-gray-500 text-sm mb-1">Expenses ({format(selectedMonth, "MMM")})</p><h3 className="text-2xl font-semibold text-red-600">-£{stats.expense.toFixed(2)}</h3></div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-blue-500"><p className="text-gray-500 text-sm mb-1">Fixed Subs</p><h3 className="text-2xl font-semibold text-blue-600">£{monthlyFixedCost.toFixed(2)}</h3></div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100"><p className="text-gray-500 text-sm mb-1">Net Balance</p><h3 className="text-2xl font-semibold text-gray-800">£{stats.balance.toFixed(2)}</h3></div>
              </div>
          </div>
        )}
        ...
        {activeTab === "daily" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm h-fit border border-gray-100 overflow-visible relative">
              ...
              <div className="space-y-4">
                ...
                <div className="relative z-10">
                  <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleAddTransaction("expense") }} className="w-full p-2 border rounded-lg outline-none" placeholder="Amount (£)" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 relative z-10">
                  ...
                  <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase">Pay With</label>
                ...
                <div className="relative z-10">
                  <label className="text-xs font-semibold text-gray-500 uppercase">Priority / Goal</label>
                  <div className="flex gap-2 mt-1">
                      {["need", "want", "save"].map((p) => (
                          <button key={p} onClick={(e) => { e.preventDefault(); setPriority(p as any); }} className={`flex-1 py-1.5 rounded-md text-xs font-semibold capitalize border transition ${priority === p ? 'bg-black text-white border-black shadow-sm' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>{p}</button>
                      ))}
                  </div>
                </div>

                <div className="relative z-10">
                  <input type="text" value={note} onChange={(e) => setNote(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleAddTransaction("expense") }} className="w-full p-2 border rounded-lg outline-none" placeholder="Note (e.g. Tesco)" />
                </div>
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative z-0">
              <div className="flex justify-between items-center mb-4">
                  <h2 className="text-lg font-semibold">Activity in {format(selectedMonth, "MMMM")}</h2>
                  <span className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-md">
                      {currentTransactions.filter(tx => selectedActivityAccount === "All" || tx.bankAccountId === selectedActivityAccount).length} items
                  </span>
              </div>
              ...
              {accounts.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4 pb-4 border-b border-gray-100">
                  <button 
                    onClick={() => setSelectedActivityAccount("All")} 
                    className={`px-4 py-1.5 rounded-full text-sm font-medium border transition ${selectedActivityAccount === "All" ? "bg-black text-white border-black" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}
                  >
                    All Cards
                  </button>
                  {accounts.map(acc => (
                    <button 
                      key={acc.id} 
                      onClick={() => setSelectedActivityAccount(acc.id)} 
                      className={`px-4 py-1.5 rounded-full text-sm font-medium border transition flex items-center gap-2 ${selectedActivityAccount === acc.id ? "bg-black text-white border-black" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}
                    >
                      <span className={`w-2 h-2 rounded-full ${acc.color.replace('bg-', 'bg-')}`}></span>
                      {acc.name}
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-6"> 
                {currentTransactions.filter(tx => selectedActivityAccount === "All" || tx.bankAccountId === selectedActivityAccount).length === 0 && (
                ...
                {(() => {
                  ...
                  return sortedDates.map(dateStr => (
                      <div key={dateStr} className="space-y-2">
                          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider pl-1 border-b border-gray-100 pb-2 mb-3 flex items-center gap-2">
                              {format(parseISO(dateStr), "do MMMM")}
                              {isToday(parseISO(dateStr)) && <span className="bg-black text-white px-2 py-0.5 rounded text-[9px] leading-none">TODAY</span>}
                          </h3>
                                      ...
                                      return (
                                          <div key={tx.id} className="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-3 shadow-inner">
                                              <div className="flex justify-between items-center mb-2">
                                                  <span className="text-xs font-semibold uppercase text-gray-500">Edit Transaction</span>
                                  ...
                                  return (
                                      <div key={tx.id} className="flex justify-between items-center p-3 hover:bg-gray-50 rounded-lg transition group border border-transparent hover:border-gray-100">
                                              ...
                                              <div>
                                                  <div className="flex items-center gap-2">
                                                      ...
                                                      {tx.type === 'expense' && tx.priority && PRIORITY_COLORS[tx.priority] && (
                                                          <span className={`text-[10px] uppercase font-semibold px-1.5 py-0.5 rounded border ${PRIORITY_COLORS[tx.priority]}`}>{tx.priority}</span>
                                                      )}
                                                  </div>
                                          ...
                                          <div className="flex items-center gap-4">
                                              <span className={`font-semibold ${tx.type === 'income' ? 'text-green-600' : 'text-gray-900'}`}>{tx.type === 'expense' ? '-' : '+'}£{tx.amount.toFixed(2)}</span>
        ...
        {activeTab === "analytics" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-300">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  <div className="flex items-center gap-2 mb-6"><BarChart3 size={20} className="text-gray-500"/><h2 className="text-xl font-semibold">Monthly History</h2></div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center">
                  <div className="flex items-center gap-2 mb-6 w-full"><PieIcon size={20} className="text-gray-500"/><h2 className="text-xl font-semibold">Category Breakdown ({format(selectedMonth, "MMM")})</h2></div>
                  <div className="h-[300px] w-full">
                    {pieData.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
        ...
        {activeTab === "subscriptions" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
              ...
              <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <h2 className="text-lg font-semibold mb-4">Your Fixed Monthly Costs</h2>
                    ...
                    <div key={sub.id} className="flex justify-between items-center p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-lg bg-gray-100 flex flex-col items-center justify-center text-gray-600 border border-gray-200">
                              ...
                              <span className="text-sm font-semibold leading-none text-black">{sub.billingDay}</span>
                          </div>
        ...
        {activeTab === "portfolio" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
            {/* Add Asset Sidebar */}
            <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm h-fit border border-gray-100">
                <h2 className="text-lg font-semibold mb-4">Record Trade</h2>
                {error && <div className="mb-4 p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center gap-2 border border-red-100"><AlertCircle size={16} /> {error}</div>}
                <div className="space-y-4">
                                ...
                                {searchResults.map((asset, i) => (
                                    <button key={i} onClick={() => handleSelectSearchResult(asset)} className="w-full text-left p-3 hover:bg-gray-50 flex items-center gap-3 border-b last:border-b-0 transition">
                                        {asset.thumb ? (
                                            ...
                                            <div className={`w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-lg text-xs font-semibold uppercase tracking-wider ${asset.type === 'crypto' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                                                {asset.symbol.substring(0, 2)}
                                            </div>
                                        )}
                                        <div className="flex flex-col overflow-hidden">
                                            <span className="font-medium text-sm text-gray-900 truncate">{asset.name}</span>
                                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                                <span className="font-mono text-black">{asset.symbol}</span>
                                                {asset.exchange && (
                                                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase ${asset.exchange === 'London' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                                                      {asset.exchange}
                                                  </span>
                                                )}
                                            </div>
            ...
            <div className="lg:col-span-2 space-y-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100 gap-4">
                    <div>
                      <h2 className="text-lg font-semibold text-gray-900">Live Holdings</h2>
                      <p className="text-sm text-gray-500">
                        Total Value: <span className="font-semibold text-black text-lg">£{totalPortfolioValue.toFixed(2)}</span>
                      </p>
                    </div>
                      ...
                      <div key={asset.symbol} className="bg-white border border-gray-200 rounded-xl shadow-sm hover:border-gray-300 transition overflow-hidden">
                        ...
                        <div onClick={() => toggleExpand(asset.symbol)} className="flex flex-col md:flex-row justify-between items-start md:items-center p-5 cursor-pointer gap-4">
                            <div className="flex items-center gap-4 relative">
                                <div className={`w-10 h-10 flex items-center justify-center rounded-xl text-xs font-semibold uppercase tracking-wider shadow-sm flex-shrink-0 ${asset.type === 'crypto' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                                    {asset.symbol.substring(0, 2)}
                                </div>
                                ...
                                <div className="ml-2"> 
                                    <h3 className="font-semibold text-lg text-gray-900 leading-tight flex items-center gap-2">
                                        {asset.name} 
                                    </h3>
                            ...
                            <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0">
                                <div className="text-left md:text-right">
                                    <p className="font-semibold text-xl text-gray-900">£{totalValue.toFixed(2)}</p>
                                    ...
                                    {asset.totalCost > 0 && currentPrice > 0 && (
                                        <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold ${isProfit ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                ...
                                {asset.lots.map((lot: any) => {
                                    ...
                                    return (
                                        <div key={lot.id} className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-200 shadow-sm text-sm">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="font-medium">{Number(lot.amount.toFixed(8))} {asset.type === 'crypto' ? 'Coins' : 'Shares'}</span>
                                            ...
                                            <div className="flex items-center gap-4">
                                                {currentPrice > 0 && (
                                                    <span className={`font-semibold ${lotProfit ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {activeTab === "cards" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
              <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm h-fit border border-gray-100">
                  <h2 className="text-lg font-semibold mb-4">Add Bank / Card</h2>
                  <div className="space-y-4">
                      <div>
                      ...
                      {accounts.map(acc => (
                          <div key={acc.id} className={`${acc.color} text-white p-6 rounded-xl shadow-md relative overflow-hidden group`}>
                              <div className="relative z-10 flex justify-between items-start">
                                  <div>
                                      <p className="text-white/70 text-sm uppercase font-semibold tracking-wider">{acc.type}</p>
                                      <h3 className="text-2xl font-semibold mt-1">{acc.name}</h3>
                                  </div>
        ...
        {activeTab === "switches" && (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
              <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm h-fit border border-gray-100">
                  <h2 className="text-lg font-semibold mb-4">Add Switch Tracker</h2>
                  {error && <div className="mb-4 p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center gap-2 border border-red-100"><AlertCircle size={16} /> {error}</div>}
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                          <div>
                              <div className="flex items-center gap-3 mb-1">
                                  <h3 className={`font-semibold text-xl ${isCompleted ? 'text-green-700' : isFailed ? 'text-red-700' : 'text-gray-900'}`}>
                                      {sw.bankName}
                                  </h3>
                                  ...
                                  {isActive && <span className="bg-blue-50 text-blue-600 text-xs px-2 py-0.5 rounded-full font-semibold flex items-center gap-1"><Clock size={12}/> In Progress</span>}
                                  
                                  <span className="bg-gray-100 text-gray-800 text-xs px-2.5 py-0.5 rounded-full font-semibold border border-gray-200">
